...
The **Forest Agreement Layer (FAL)** is a set of open access [Google Earth Engine](https://earthengine.google.com/) (GEE) scripts that merges global forest datasets (currently up to 9) into one single map, separating steps requiring user input from automated tasks. The term *forest agreement* refers to a multi-source blend of global forest and tree-cover datasets to highlight where maps agree or disagree on forest/tree cover. 

A FAL can be for example used for due diligence purposes or to monitor compliance relating to the [Regulation on Deforestation-free Products](https://environment.ec.europa.eu/topics/forests/deforestation/regulation-deforestation-free-products_en) **(EUDR)**, as it refers to the cut-off year and the definitions set out in the regulation. It can also be used to monitor compliance with deforestation-free requirements that fall outside the scope of the EUDR, or for other purposes defined by the user.
