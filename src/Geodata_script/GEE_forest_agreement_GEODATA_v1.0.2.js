...
var MMU_HECTARES = 0.5;           // Minimum Mapping Unit (MMU) in hectares used during filtering
var FOREST_HEIGHT_MIN = 5;        // Minimum forest height in meters set for tree height datasets
...
var NatF_THRESHOLD = 0.52; //OA optimal probability threshold of 0.52 (stated by the Authors)
...
// Threshold-based (Probability) forest/non-forest reclassification
function reclassifyThreshold(image, threshold) {
...
// JRC (10m) 2020 version 3 [Bourgoin et al., 2025] 
var jrc_2020 = ee.Image("JRC/GFC2020/V3").clip(roi);
...
// Natural Forests of the World 2020 - Probabilistic map [Neumann et al. 2025]
var NFprob = ee.ImageCollection('projects/nature-trace/assets/forest_typology/natural_forest_2020_v1_0_collection')
        .mosaic()
        .select('B0');
//B0 	0 	250 	10 meters 	Natural forest probabilities (scaled to [0-250]).        


//Global Satellite Embedding-based Map of Forests and Tree Crops 2020 (10m) version 1.0 [Paluba et al. 2026]
...
var reclassifiedList = [
  ...
  reprojectAndResample(reclassifyThreshold(NFprob, NatF_THRESHOLD)),
  reprojectAndResample(reclassifyImage(GEM_Forest, forestClasses['GEM']))
];
...
// Combine all 11 reclassified forest masks
...
var SIEVE_THRESHOLD_PIXELS = ee.Number(MMU_HECTARES)
  .multiply(10000) // convert ha → m²
  .divide(ee.Number(TARGET_RESOLUTION).pow(2))
  .round(); // or ceil(), see note below
...
var visParams = {
  ...
  '#FF8C00', // dark orange
  '#FFA500', // orange
  '#FFD580', // light orange / peach
  '#FFFF00', // yellow
  '#CCCC00', // dark yellow / olive
  '#90EE90', // light green
  '#32CD32', // lime green
  '#006400',  // dark green
...
var labels = [
  '0 maps', '1 map', '2 maps', '3 maps', '4 maps',
...
var forestLayers = [
  ...
  {name: 'NatF',          image: reclassifiedList[9]},
  {name: 'GEM',          image: reclassifiedList[10]}
];
...
// --- 3) Compute pixel-area image and mask for values 6..11 ---
var pixelArea = ee.Image.pixelArea(); // m^2 per pixel

// Create binary mask image (1 where 6..11, else 0) = MAJORITY OF MAPS
var mask6to11 = forestAgreementFiltered.gte(6).and(forestAgreementFiltered.lte(11)).rename('mask6to11').updateMask(ee.Image.constant(1));
var maskAreaImg = mask6to11.multiply(pixelArea).rename('area_6to11_m2');

// --- 4) Reduce: sum of area_6to11_m2 within each polygon ---
...
var withPercent = stats.map(function(feature) {
  var area6_11_m2 = ee.Number(feature.get('sum')).max(0); // area in m2 for classes 6..9
  ...
  percent = ee.Algorithms.If(poly_area_m2.gt(0),
                             area6_11_m2.divide(poly_area_m2).multiply(100),
                             0);
  return feature.set({
...
// Add median forest agreement class (fal_class) per polygon
var final_with_fal = final.map(function(feature) {

  var medianAgreement = forestAgreementFiltered.reduceRegion({
  ...
  var fal_class = ee.Number(medianAgreement).round();
...
// Replace final with updated collection
final = final_with_fal;
...
Map.addLayer(final, {}, 'Final Polygons');


// DEALING WITH SHAPEFILE CONTAINING MULTIPLE GEOMETRIES
// Only VALID geometries (no mixing geometries)
var final_clean = final.map(function(f) {
  return f.set('geom_type', f.geometry().type());
});

var polygons_only = final_clean.filter(ee.Filter.eq('geom_type', 'Polygon'));

//Map.addLayer(polygons_only, {}, 'New Polygons');

// Export the full geometries and attributes as a shapefile.
Export.table.toDrive({
  collection: polygons_only,
